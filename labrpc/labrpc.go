...
type ClientEnd struct {
	...
	ch      chan reqMsg   // copy of Network.endCh 消息总线
	done    chan struct{} // closed when Network is cleaned up 网络销毁通道
	callf   Fcall //测试专用钩子
}
//自定义回调
...
//KVServer/Raft节点收到客户端RPC，中转发给其他节点
...
///Makenetwork启动的匿名gorouting收到消息使用这个去处理
func (rn *Network) processReq(req reqMsg) {
	...
	if enabled && servername != nil && server != nil {
		...
		if replyOK == false || serverDead == true {
			// 情况1：服务没返回 / 服务器中途被销毁 → 返回失败
			req.replyCh <- replyMsg{false, nil}
		} else if reliable == false && (rand.Int()%1000) < 100 {
			// 情况2：10%概率丢弃响应包（请求到达服务、业务执行成功，但回复丢在路上）
			req.replyCh <- replyMsg{false, nil}
		} else if longreordering == true && rand.Intn(900) < 600 {
			// 情况3：开启乱序，60%回复延迟200~2200ms再发回客户端
...
//seek client where name is endname 
